...
function renderSinglePost(data) {
	...
	html += '<div class="central-meta item" id="post-' + data._id + '">';
		html += '<div class="user-post">';
			...
			if (isMyUploaded) {
				html += "<div class='row'>"
					html += "<div class='offset-md-7 col-md-5'>"
						if (data.isBoost) {
							html += "<a href='javascript:void(0);' style='background-color: gray; color: white; padding: 5px;' class='pull-right'>Boost post</a>"
						} else {
							html += "<a href='/boostPost/" + data._id + "' style='background-color: #088dcd; color: white; padding: 5px;' class='pull-right'>Boost post <span class='txt-premium' style='margin-left: 10px; color: greenyellow;'></span></a>"
						}
					html += "</div>"
				html += "</div>"
			}
					...
					html += '<form method="post" id="form-add-post" onsubmit="return doPost(this);" novalidate enctype="multipart/form-data">';
						...
						html += '<div class="attachments">';
							html += '<ul>';
								...
								html += '<li>';
									html += '<audio id="post-audio-preview" controls style="display: none; margin-bottom: 10px;"></audio>';
								html += '</li>';

								html += '<li>';
									html += '<p id="post-document-preview"></p>';
								html += '</li>';

								html += '<li>';
									html += '<i class="fa fa-image"></i>';
									html += '<label class="fileContainer">';
										html += '<input type="file" name="image" accept="image/*" onchange="previewPostAttachment(this, \'post-img-preview\');">';
									html += '</label>';
								html += '</li>';
								...
								html += '<li>';
									...
									html += '<label class="fileContainer">';
										html += '<input type="file" name="video" accept="video/*" onchange="previewPostAttachment(this, \'post-video-preview\');">';
									html += '</label>';
								html += '</li>';
								...
								html += '<li>'
									...
									html += '<label class="fileContainer">'
										html += '<input type="file" name="audio" accept=".mp3,.m4a,.aac" onchange="previewPostAttachment(this, \'post-audio-preview\');">'
									html += '</label>'
								html += '</li>'
								...
								html += '<li>'
									...
									html += '<label class="fileContainer">'
										html += '<input type="file" name="document" accept=".xls,.xlsx,.pdf,.doc,.docx,.ppt" onchange="previewPostAttachment(this, \'post-document-preview\');" />'
									html += '</label>'
								html += '</li>'*/
